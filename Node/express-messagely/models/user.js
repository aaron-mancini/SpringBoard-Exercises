...
class User {
  ...
  static async updateLoginTimestamp(username) {
    ...
    if (!result.rows[0]) {
      throw new ExpressError(`Failed to update user. ${username} does not exist`, 404);
    }
  }
  ...
  static async messagesTo(username) {
    const result = await db.query(
      `SELECT m.id, m.from_user, u.first_name, u.last_name, u.phone, m.body, m.sent_at, m.read_at
      FROM messages AS m
      JOIN users AS u
