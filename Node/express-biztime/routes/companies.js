...
router.get("/:code", async (req, res, next) => {
    try {
        const results = await db.query(`SELECT c.code, c.name, c.description, i.industry FROM companies AS c
                                        LEFT JOIN company_industries AS ci
                                        ON c.code = ci.company_code
                                        LEFT JOIN industries AS i 
                                        ON ci.industry_code = i.code
                                        WHERE c.code = $1`, [req.params.code]);
        const compInvoices = await db.query(`SELECT * FROM invoices WHERE comp_code=$1`, [req.params.code]);
        ...
        return res.send({ company: code, name, description, industries, invoices: compInvoices.rows });
    } catch (e) {
        return next(e);
    }
})
